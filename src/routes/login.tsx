...
function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(false);
    if (error) toast.error(t("שגיאת התחברות: ") + error.message);
    else {
      toast.success(t("ברוך הבא לזוגה"));
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    setLoading(false);
    if (error) toast.error(t("שגיאת הרשמה: ") + error.message);
    else toast.success(t("נרשמת בהצלחה. בדוק את האימייל לאישור."));
  }
  ...
  async function handleGoogle() {
    ...
    if (result.error) {
      setLoading(false);
      toast.error(t("שגיאת התחברות עם גוגל: ") + (result.error as Error).message);
      return;
    }
    ...
    setLoading(false);
    if (!data.session) {
      toast.error(t("ההתחברות לא הושלמה. נסה שוב."));
      return;
    }
    window.location.href = returnTo;
  }
      ...
      <Card className="w-full max-w-md p-8 shadow-lg" style={{ boxShadow: "var(--shadow-warm)" }}>
        ...
        <Tabs defaultValue="login">
          <TabsList className="grid w-full grid-cols-2 mb-4">
            <TabsTrigger value="login">{t("התחברות")}</TabsTrigger>
            <TabsTrigger value="signup">{t("הרשמה")}</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="login">
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <Label htmlFor="email">{t("אימייל")}</Label>
                <Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="password">{t("סיסמה")}</Label>
                <Input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)} />
              </div>
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? t("מתחבר...") : t("התחבר")}
              </Button>
            </form>
          </TabsContent>
          ...
          <TabsContent value="signup">
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                <Label htmlFor="email2">{t("אימייל")}</Label>
                <Input id="email2" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
              <div>
                <Label htmlFor="password2">{t("סיסמה")}</Label>
                <Input id="password2" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} />
              </div>
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? t("נרשם...") : t("צור חשבון מנהל")}
              </Button>
              <p className="text-xs text-muted-foreground text-center">
                {t("המשתמש הראשון נרשם אוטומטית כמנהל")}
              </p>
            </form>
          </TabsContent>
        </Tabs>
