...
const RESET_RE =
  /(נתחיל\s+מחדש|נתחיל\s+שוב|מתחילים\s+שוב|התחל\s+מחדש|התחל\s+שוב|בוא(י)?\s+נתחיל\s+(מחדש|שוב)|להתחיל\s+(מחדש|שוב)|התחלה\s+חדשה|מתחילים\s+מחדש|אפס(י)?\s+(את\s+)?השיחה|לאפס\s+(את\s+)?השיחה|תתחילי\s+(מחדש|שוב)|נתחיל\s+מהתחלה|מהתחלה\s+בבקשה|^\s*ריסט\s*$|^\s*רסט\s*$|start\s+over|^\s*reset\s*$|reset\s+(the\s+)?(chat|conversation))/i;
