...
- חשוב: הדף עשוי להכיל מספר הצעות (קטלוג שלם / דף בית). חלץ עובדות אך ורק עבור ההצעה המבוקשת — לפי "כותרת ההצעה" ו"קטגוריה" שמועברים אליך. התעלם מהצעות אחרות באותו דף.
- אם בדף יש סקציה ייעודית להצעה המבוקשת (למשל בלוק "אלבניה" עם מחיר ותוספת ליחיד) — חלץ אותה. אל תערבב מחירים בין יעדים שונים.
- "published" מותר גם כאשר המחיר נמצא בתוך דף רב-יעדי, כל עוד הוא משויך באופן ברור להצעה המבוקשת.
...
כלל קריטי לתמחור: status חייב לשקף את המציאות. אם תכתוב status="published" בלי base_price_per_person אמיתי מהדף — זו טעות חמורה. אם הדף הוא דף בית בלי שום אזכור של ההצעה המבוקשת — status="unpublished". אם הדף מכיל מחיר ספציפי להצעה המבוקשת (גם בתוך דף רב-יעדי) — status="published".`;
