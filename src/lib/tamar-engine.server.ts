/**
 * TAMAR ENGINE — the ONLY runtime brain for live inbound turns.
 *
...
export type TamarTurnResult = { status: number; payload: any };

/**
 * THE Tamar engine. Single owner of a live inbound turn:
 * contact resolve/create -> context load -> prompt build -> Lovable AI call
...
export async function runTamarTurn(body: any): Promise<TamarTurnResult> {
  ...
  if (!message) return { status: 400, payload: { ok: false, error: "message_required" } };
  ...
  } = { manager_available: false, delivery_promise: "queued" };
  if (handoffLikelyThisTurn) {
    // Zooga-only handoff: there is NO live manager transport. A handoff is
    // always QUEUED (manager_handoffs row + ops task + attention flag).
    const { data: managerPre } = await supabaseAdmin
      ...
      .limit(1)
      .maybeSingle();
    handoffPreCheck = { manager_available: !!managerPre, delivery_promise: "queued" };
    replyHardRules.push(
      "Handoff is QUEUED, never live. You MUST NOT say 'מעבירה אותך עכשיו' / 'I'm transferring you now' / any present-tense live-transfer phrasing. Use FUTURE/QUEUED tense ONLY: 'אעביר את הפרטים לצוות וייצרו איתך קשר בהקדם' or equivalent.",
    );
  }
